    ...
    public function getStudentProspective(Request $request)
    {
        ...
        return Datatables::of($students_prospective)
            ->addColumn('action', function ($row) { 
                ...
                $approve = '<form action="' . url('update_status', $row->stu_id) . '" method="post">
                                <input type="submit" class="btn btn-outline-primary waves-effect waves-light m-1"> <i class="zmdi zmdi-info-outline fa-lg"></i> />
                            </form>';
